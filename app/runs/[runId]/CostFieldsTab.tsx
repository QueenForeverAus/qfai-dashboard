...
import { OwnerPnlSummary, OwnerRevenueBlock, type OwnerVenueRevenue } from '@/components/RunCostingOwnerPnl'
import {
  canSeeOwnerPnlSheet,
...
export type RemittanceInsideLine = {
  show_id: string | null
...
export default function CostFieldsTab({
  ...
  insideFactorRows?: Array<{ key: string; value: unknown }>
  remittanceInsideLines?: RemittanceInsideLine[]
}) {
  ...
  const showOwnerPnl = canSeeOwnerPnlSheet(effectiveRole)
  ...
  const [activeTab, setActiveTab] = useState<'costs' | 'outlook' | 'audit' | 'advancement' | 'show_pack'>(
    defaultTab as 'costs' | 'outlook' | 'audit' | 'advancement' | 'show_pack',
  )
  const insideFactors = parseInsideFactors(insideFactorRows)

  useEffect(() => {
    if (typeof window === 'undefined') return
    const params = new URLSearchParams(window.location.search)
    const tab = params.get('tab')
    if (tab === 'overview' || tab === 'pnl' || tab === 'calculator') {
      setActiveTab('costs')
      params.delete('tab')
  ...
  function insideLinesForField(row: CostFieldRow | undefined): KnownInsideLine[] {
    if (!row) return []
    const lines: KnownInsideLine[] = []
    for (const entry of row.entries ?? []) {
      lines.push({ description: entry.description, notes: entry.notes, amount: entry.amount })
    }
    if (row.field_key === 'venue_staff') {
      for (const item of row.line_items ?? []) {
        const amount = (Number(item.rate) || 0) * (Number(item.hours) || 0) * (Number(item.headcount) || 0)
        lines.push({ description: item.role, notes: item.source, amount })
      }
    }
    return lines
  }

  function insideAmountOnField(row: CostFieldRow | undefined): number {
    return sumKnownInside(insideLinesForField(row)) ?? 0
  }

  function outsideFieldValue(row: CostFieldRow | undefined, show?: Show, pct?: number): number {
    if (!row) return 0
    const raw = show && row.field_key === 'venue_staff'
      ? (calcVenueStaff(show, pct ?? 75, row) ?? 0)
      : (effectiveFieldValue(row) ?? 0)
    return Math.max(0, raw - insideAmountOnField(row))
  }
  ...
  const runCostTotal = RUN_FIELDS.reduce((sum, f) => {
    ...
    return sum + outsideFieldValue(row)
  }, 0)
  ...
  const showCostTotal = showsState.reduce((sum, show) => {
    ...
    return sum + SHOW_FIELDS.filter(f => f.category !== 'Revenue').reduce((s2, f) => {
      ...
      return s2 + outsideFieldValue(row, show, pct)
    }, 0)
  }, 0)
  ...
  const unlockFields = [
    ...RUN_FIELDS.map(f => {
      ...
      return {
        ...
        state: row?.state,
        label: f.label,
        paidAll: allEntriesPaid(sectionPayableLines(f.key, row?.entries, row?.line_items)),
      }
    }),
    ...showsState.flatMap(show =>
      SHOW_FIELDS.map(sf => {
        ...
        return {
          ...
          state: row?.state,
          label: sf.label,
          showLabel: show.venue_city,
          paidAll: allEntriesPaid(sectionPayableLines(sf.key, row?.entries, row?.line_items)),
        }
      }),
    ),
  ]
  const unlock = sliderUnlock(unlockFields)
  const incompleteFields = unlock.blocking
  const isDataComplete = unlock.unlocked
  const hasGuessFields = !isDataComplete ? false : unlockFields.some(f => {
    if (f.field_key === 'social_ads_var' || f.field_key === 'gross_box_office' || f.category === 'Revenue') return false
    return f.state === 'guess' || f.state === 'estimated'
  })

  const ownerVenues: OwnerVenueRevenue[] = showsState.map(show => {
    const pct = sellThrough[show.id] ?? 75
    const cap = modelCapacity(show)
    const tickets = cap ? Math.round(cap * pct / 100) : 0
    const gross = projectedBoxOffice(show, pct) ?? 0
    const contractInside = sumKnownInside(
      SHOW_FIELDS.filter(f => f.category !== 'Revenue').flatMap(f =>
        insideLinesForField(fieldMap.get(showFieldKey(show.id, f.key))),
      ),
    )
    const remittanceInside = knownInsideFromRemittance(remittanceInsideLines, show.id)
    const knownInside = remittanceInside ?? contractInside
    ...
    return {
      ...
      sellThroughPct: pct,
      tickets: cap ? tickets : null,
      waterfall: revenueWaterfall(gross, resolved.inside),
      insideSource: resolved.source,
    }
  })
  const ownerNetRevenue = ownerVenues.reduce((sum, v) => sum + v.waterfall.netRevenue, 0)
  const ownerPnl = pnlSummary(ownerNetRevenue, totalCosts)
  ...
  return (
    <>
      ...
      {/* COST FIELDS TAB — owner top/bottom wrap existing middle */}
      {hasTabAccess && activeTab === 'costs' && (
        <div className="space-y-6">
          {showOwnerPnl && (
            <OwnerRevenueBlock
              venues={ownerVenues}
              slidersUnlocked={isDataComplete}
          ...
          {showOwnerPnl && (
            <OwnerPnlSummary summary={ownerPnl} unlocked={isDataComplete} />
          )}
        </div>
      )}
